...
export default function KeywordResearchTool({ projectId, projectName }: KeywordResearchToolProps) {
  ...
  const [keywordResults, setKeywordResults] = useState<Record<string, KeywordSuggestion[]>>({
    mainKeywords: [],
    relatedKeywords: [],
    longTailKeywords: [],
    questions: [],
  ...
  const activeKeywords = keywordResults[activeTab as keyof typeof keywordResults] || [];

  // Handle keyword selection
  const toggleKeywordSelection = (keyword: string, category: string) => {
    setSelectedKeywords(prev => {
      const newSelection = { ...prev };
      const categorySet = new Set(newSelection[category as keyof typeof newSelection]);
  ...
  const handleSaveKeywords = async () => {
    ...
    try {
      ...
      const keywordsToSave = Object.entries(selectedKeywords).flatMap(([category, keywordSet]) => {
        return Array.from(keywordSet).map((keyword) => {
          const keywordData = keywordResults[category as keyof typeof keywordResults]
      ...
      const response = await fetch(`/api/projects/${projectId}/keywords/bulk-add`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to save keywords. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Generate keyword ideas for {projectName ? `"${projectName}"` : "your project"} using advanced AI. Our system analyzes search patterns and competition to suggest the most valuable keywords.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {!isLoading && activeKeywords.length > 0 && (
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Found {Object.values(keywordResults).flat().length} keywords across 4 categories. Select keywords to add to your project.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="mainKeywords" value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="grid grid-cols-4 mb-4">
                {KEYWORD_CATEGORIES.map(category => (
                  ...
                  >
                    {category.label}
                    {selectedKeywords[category.id as keyof typeof selectedKeywords].size > 0 && (
                      <Badge 
                        variant="secondary" 
                        className="ml-2 absolute -top-2 -right-2 h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs"
                      >
                        {selectedKeywords[category.id as keyof typeof selectedKeywords].size}
                      </Badge>
                    )}
                  </TabsTrigger>
                ))}
                  ...
                  <ScrollArea className="h-[350px]">
                    <table className="w-full">
                      <thead className="bg-muted sticky top-0">
                        <tr>
                          <th className="text-left py-2 px-3 text-xs font-medium">Select</th>
                          <th className="text-left py-2 px-3 text-xs font-medium">Keyword</th>
                          <th className="text-right py-2 px-3 text-xs font-medium">Volume</th>
                          <th className="text-right py-2 px-3 text-xs font-medium">Difficulty</th>
                          <th className="text-right py-2 px-3 text-xs font-medium">CPC ($)</th>
                        </tr>
                      </thead>
                      <tbody>
                        {keywordResults[category.id]?.map((keyword, idx) => (
                          ...
                          >
                            <td className="py-2 px-3">
                              <Checkbox 
                                checked={selectedKeywords[category.id as keyof typeof selectedKeywords].has(keyword.keyword)}
                                onCheckedChange={() => toggleKeywordSelection(keyword.keyword, category.id)}
                                id={`${category.id}-${idx}`}
                              />
                            </td>
                            <td className="py-2 px-3">
                            ...
                            <td className="py-2 px-3 text-right">
                              <span className={`inline-block w-12 text-center rounded-full px-2 py-0.5 text-xs ${
                            ...
                            <td className="py-2 px-3 text-right">
                              ${typeof keyword.cpc === 'number' ? keyword.cpc.toFixed(2) : "N/A"}
                            </td>
                          </tr>
                        ))}
